...
import type { Foerderantrag, CreateFoerderantrag } from '@/types/app';
...
export class LivingAppsService {
  // --- FOERDERANTRAG ---
  static async getFoerderantrag(): Promise<Foerderantrag[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.FOERDERANTRAG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Foerderantrag[];
    return enrichLookupFields(records, 'foerderantrag');
  }
  static async getFoerderantragEntry(id: string): Promise<Foerderantrag | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.FOERDERANTRAG}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Foerderantrag;
    return enrichLookupFields([record], 'foerderantrag')[0];
  }
  static async createFoerderantragEntry(fields: CreateFoerderantrag): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.FOERDERANTRAG}/records`, { fields: cleanFieldsForApi(fields as any, 'foerderantrag') });
    return { ...data, record_id: data.id };
  }
  static async updateFoerderantragEntry(id: string, fields: Partial<CreateFoerderantrag>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.FOERDERANTRAG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'foerderantrag') });
    return { ...data, record_id: data.id };
  }
  static async deleteFoerderantragEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.FOERDERANTRAG}/records/${id}`);
  }
