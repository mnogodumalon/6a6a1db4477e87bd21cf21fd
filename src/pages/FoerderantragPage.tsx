...
export default function FoerderantragPage() {
  ...
  return (
    <PageShell
      title="Förderantrag"
      subtitle={`${records.length} Förderantrag im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Förderantrag suchen..."
          value={search}
          ...
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('anzahl_beguenstigte')}>
                <span className="inline-flex items-center gap-1">
                  Voraussichtliche Anzahl der Begünstigten
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('vorherige_foerderung')}>
                <span className="inline-flex items-center gap-1">
                  Wurde das Projekt bereits gefördert?
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('vorherige_foerderung_beschreibung')}>
                <span className="inline-flex items-center gap-1">
                  Angaben zur bisherigen Förderung
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('drittmittel')}>
                <span className="inline-flex items-center gap-1">
                  Drittmittel / weitere Fördermittel (€)
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/foerderantrag/${record.record_id}`); }}>
                ...
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{record.fields.bundesland?.label ?? '—'}</span></TableCell>
                ...
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{record.fields.projektkategorie?.label ?? '—'}</span></TableCell>
                ...
                <TableCell><span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${record.fields.bereits_durchgefuehrt ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}>{record.fields.bereits_durchgefuehrt ? 'Ja' : 'Nein'}</span></TableCell>
                <TableCell><span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${record.fields.vorherige_foerderung ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}>{record.fields.vorherige_foerderung ? 'Ja' : 'Nein'}</span></TableCell>
                ...
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{record.fields.foerderart?.label ?? '—'}</span></TableCell>
                <TableCell>{Array.isArray(record.fields.anlagen) ? record.fields.anlagen.map((v: any) => v?.label ?? v).join(', ') : '—'}</TableCell>
                ...
                <TableCell><span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${record.fields.datenschutz ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}>{record.fields.datenschutz ? 'Ja' : 'Nein'}</span></TableCell>
                <TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={41} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Förderantrag. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
