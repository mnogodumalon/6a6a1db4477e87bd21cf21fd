...
import { FoerderantragViewDialog } from '@/components/dialogs/FoerderantragViewDialog';
...
const FOERDERANTRAG_FIELDS = [
  { key: 'anrede', label: 'Anrede', type: 'lookup/radio', options: [{ key: 'herr', label: 'Herr' }, { key: 'frau', label: 'Frau' }, { key: 'divers', label: 'Divers' }, { key: 'organisation', label: 'Organisation' }] },
  { key: 'vorname', label: 'Vorname', type: 'string/text' },
  { key: 'nachname', label: 'Nachname', type: 'string/text' },
  { key: 'organisation_name', label: 'Name der Organisation / Institution', type: 'string/text' },
  { key: 'rechtsform', label: 'Rechtsform', type: 'lookup/select', options: [{ key: 'einzelperson', label: 'Einzelperson' }, { key: 'verein', label: 'Verein (e. V.)' }, { key: 'gmbh', label: 'GmbH' }, { key: 'ag', label: 'AG' }, { key: 'stiftung', label: 'Stiftung' }, { key: 'koerperschaft', label: 'Körperschaft des öffentlichen Rechts' }, { key: 'sonstige', label: 'Sonstige' }] },
  ...
  { key: 'projekttitel', label: 'Projekttitel', type: 'string/text' },
  { key: 'projektkategorie', label: 'Projektkategorie', type: 'lookup/select', options: [{ key: 'bildung', label: 'Bildung & Forschung' }, { key: 'soziales', label: 'Soziales & Integration' }, { key: 'umwelt', label: 'Umwelt & Nachhaltigkeit' }, { key: 'kultur', label: 'Kultur & Kunst' }, { key: 'digitalisierung', label: 'Digitalisierung & Innovation' }, { key: 'gesundheit', label: 'Gesundheit & Sport' }, { key: 'wirtschaft', label: 'Wirtschaft & Beschäftigung' }, { key: 'sonstiges', label: 'Sonstiges' }] },
  { key: 'projektbeschreibung', label: 'Projektbeschreibung', type: 'string/textarea' },
  { key: 'zielgruppe', label: 'Zielgruppe', type: 'string/textarea' },
  { key: 'projektziele', label: 'Projektziele', type: 'string/textarea' },
  { key: 'projektstart', label: 'Geplanter Projektbeginn', type: 'date/date' },
  { key: 'projektende', label: 'Geplantes Projektende', type: 'date/date' },
  { key: 'projektort', label: 'Durchführungsort', type: 'string/text' },
  { key: 'anzahl_beguenstigte', label: 'Voraussichtliche Anzahl der Begünstigten', type: 'number' },
  { key: 'bereits_durchgefuehrt', label: 'Wurde das Projekt bereits begonnen?', type: 'bool' },
  { key: 'vorherige_foerderung', label: 'Wurde das Projekt bereits gefördert?', type: 'bool' },
  { key: 'vorherige_foerderung_beschreibung', label: 'Angaben zur bisherigen Förderung', type: 'string/textarea' },
  { key: 'gesamtkosten', label: 'Gesamtkosten des Projekts (€)', type: 'number' },
  { key: 'beantragte_foerdersumme', label: 'Beantragte Fördersumme (€)', type: 'number' },
  { key: 'eigenanteil', label: 'Eigenanteil (€)', type: 'number' },
  { key: 'drittmittel', label: 'Drittmittel / weitere Fördermittel (€)', type: 'number' },
  { key: 'drittmittel_herkunft', label: 'Herkunft der Drittmittel', type: 'string/textarea' },
  { key: 'verwendungszweck', label: 'Geplante Mittelverwendung', type: 'string/textarea' },
  { key: 'foerderart', label: 'Art der Förderung', type: 'lookup/radio', options: [{ key: 'zuschuss', label: 'Zuschuss' }, { key: 'darlehen', label: 'Darlehen' }, { key: 'sachleistung', label: 'Sachleistung' }, { key: 'sonstiges_foerderart', label: 'Sonstiges' }] },
  { key: 'anlagen', label: 'Beizufügende Unterlagen', type: 'multiplelookup/checkbox', options: [{ key: 'projektplan', label: 'Projektplan' }, { key: 'kostenaufstellung', label: 'Kostenaufstellung' }, { key: 'vereinsregister', label: 'Vereinsregisterauszug' }, { key: 'handelsregister', label: 'Handelsregisterauszug' }, { key: 'satzung', label: 'Satzung' }, { key: 'jahresabschluss', label: 'Jahresabschluss / Bilanz' }, { key: 'referenzen', label: 'Referenzen / Nachweise' }, { key: 'sonstige_unterlagen', label: 'Sonstige Unterlagen' }] },
  { key: 'dateiupload', label: 'Dokumente hochladen', type: 'file' },
  { key: 'ansprechpartner_vorname', label: 'Vorname der Ansprechperson', type: 'string/text' },
  { key: 'ansprechpartner_nachname', label: 'Nachname der Ansprechperson', type: 'string/text' },
  { key: 'ansprechpartner_telefon', label: 'Telefon der Ansprechperson', type: 'string/tel' },
  { key: 'ansprechpartner_email', label: 'E-Mail der Ansprechperson', type: 'string/email' },
  { key: 'bemerkungen', label: 'Weitere Anmerkungen', type: 'string/textarea' },
  { key: 'datenschutz', label: 'Ich habe die Datenschutzhinweise gelesen und stimme der Verarbeitung meiner Daten zu.', type: 'bool' },
];

const ENTITY_TABS = [
  { key: 'foerderantrag', label: 'Förderantrag', pascal: 'Foerderantrag' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('foerderantrag');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'foerderantrag': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'foerderantrag': return (data as any).foerderantrag as Foerderantrag[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'foerderantrag': return FOERDERANTRAG_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'foerderantrag': return {
        create: (fields: any) => LivingAppsService.createFoerderantragEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateFoerderantragEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteFoerderantragEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'foerderantrag' || dialogState?.entity === 'foerderantrag') && (
        <FoerderantragDialog
          open={createEntity === 'foerderantrag' || dialogState?.entity === 'foerderantrag'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'foerderantrag' ? handleUpdate : (fields: any) => handleCreate('foerderantrag', fields)}
          defaultValues={dialogState?.entity === 'foerderantrag' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Foerderantrag']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Foerderantrag']}
        />
      )}
      {viewState?.entity === 'foerderantrag' && (
        <FoerderantragViewDialog
          open={viewState?.entity === 'foerderantrag'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'foerderantrag', record: r }); }}
        />
      )}
